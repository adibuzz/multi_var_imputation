...
def train_gruode(simulation_name, params_dict, device, train_idx, val_idx, epoch_max):
    ...
    np.save(rf"mimic_fold_idx_{fold}\{simulation_name}_params.npy", params_dict)

    nnfwobj = models.NNFOwithBayesianJumps(input_size=params_dict["input_size"], hidden_size=params_dict["hidden_size"],
    ...
    for epoch in range(epoch_max):
        ...
        with torch.no_grad():
            ...
            if val_metric > val_metric_prev:
                ...
                print("Saving Model")
                torch.save(nnfwobj.state_dict(), rf"mimic_fold_idx_{fold}\{simulation_name}_MAX.pt")
                val_metric_prev = val_metric
            else:
                if epoch % 10:
                    torch.save(nnfwobj.state_dict(), rf"mimic_fold_idx_{fold}\{simulation_name}.pt")
...
if __name__ == "__main__":
    ...
    val_idx = np.load(rf"mimic_fold_idx_{fold}/val_idx.npy", allow_pickle=True)

    # Model parameters.
    params_dict = dict()
    params_dict["csv_file_path"] = "Item3.csv"
    ...
    params_dict["T_val"] = 40
    params_dict["max_val_samples"] = 3

    info, val_metric_prev = train_gruode(simulation_name=simulation_name,
